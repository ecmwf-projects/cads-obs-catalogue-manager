...
TABLES_FILE="${PROJECT_ROOT}/tests/scripts/gnss_tables.txt"
IFS=$'\n'
# Number of lines
LIMIT="500"
#
...
function dump_data_table_with_limit(){
  ...
  psql  -U ${INGESTION_DB_USER} -h localhost -p ${INGESTION_DB_LOCAL_PORT} -d ${db_name} \
    -c 'COPY (SELECT * FROM '${table_name}' ORDER BY id LIMIT '${LIMIT}') TO STDOUT;' > /tmp/${table_name}_data.tsv
}
...
function dump_ingestion_tables_data_with_limit(){
  ...
  psql  -U ${INGESTION_DB_USER} -h localhost -p ${INGESTION_DB_LOCAL_PORT} -d ${db_name} \
    -c 'COPY (SELECT * FROM '${data_table_to_use}' ORDER BY id LIMIT '${LIMIT}') TO STDOUT;' > /tmp/${data_table}_data.tsv
  # Dump the related headers with a subquery
  psql  -U ${INGESTION_DB_USER} -h localhost -p ${INGESTION_DB_LOCAL_PORT} -d ${db_name} \
    -c 'COPY (SELECT * FROM '${header_table}' WHERE '${header_index}'
    in (SELECT '${header_index_in_data}' FROM '${data_table_to_use}' ORDER BY id LIMIT '${LIMIT}')) TO STDOUT;' > /tmp/${header_table}_data.tsv
}
...
cat /tmp/roles.sql /tmp/functions.sql /tmp/*_schema_header.sql  /tmp/*_schema_data.sql | \
  grep -v "CREATE TRIGGER" |  sed s/'PARTITION BY RANGE (date_of_observation)'// > "${OUTPUT_FILE}"
rm /tmp/*.sql
